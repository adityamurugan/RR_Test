...
namespace SBPScripts
{
    ...
    public class BicycleController : MonoBehaviour
    {
        ...
        WebSocket ws;
        public string cycStatus;
        public float steerzoAngle;
        ...
        void Start()
        {
            ...
            ws.OnMessage += (sender, e) =>
            {
                ...
                data.TryGetValue("status", out string status);
                data.TryGetValue("steer", out string steerAng);
                Debug.Log($"{status}");
                cycStatus = status;
                steerzoAngle = float.Parse(steerAng);
            };
        ...
        void FixedUpdate()
        {
            ...
            fPhysicsWheel.transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y + steerzoAngle * steerAngle.Evaluate(rb.velocity.magnitude) + oscillationSteerEffect, 0);
            fPhysicsWheelConfigJoint.axis = new Vector3(1, 0, 0);
            // Debug.Log(customSteerAxis);
            ...
            if (currentSpeed < currentTopSpeed && cycStatus == "cycling")
            ...
            if (currentSpeed < currentTopSpeed && cycStatus == "cycling" && !isAirborne && !isBunnyHopping)
            ...
            if ((currentSpeed < reversingSpeed || transform.InverseTransformDirection(rb.velocity).z > 0) && cycStatus == "reverse" && !isAirborne && !isBunnyHopping)
                rb.AddForce(-transform.forward * accelerationCurve.Evaluate(customAccelerationAxis) * 2f);
            ...
            if (cycStatus == "stopped" && isReversing == false && !isAirborne && !isBunnyHopping)
            ...
            cycleGeometry.handles.transform.localRotation = Quaternion.Euler(0, steerzoAngle * steerAngle.Evaluate(currentSpeed) + oscillationSteerEffect * 5, 0) * initialHandlesRotation;
            ...
            cycleGeometry.lowerFork.transform.localRotation = Quaternion.Euler(0, steerzoAngle * steerAngle.Evaluate(currentSpeed) + oscillationSteerEffect * 5, steerzoAngle * -axisAngle) * initialLowerForkLocalRotaion;
            ...
            cycleGeometry.fWheelVisual.transform.rotation = Quaternion.Euler(xQuat * (steerzoAngle * -axisAngle), steerzoAngle * steerAngle.Evaluate(currentSpeed) + oscillationSteerEffect * 5, zQuat * (steerzoAngle * -axisAngle));            
            ...
            if (Physics.Raycast(transform.position + new Vector3(0, 1f, 0), Vector3.down, out hit, Mathf.Infinity))
            {
                ...
                if (hit.distance > airTimeSettings.heightThreshold && airTimeSettings.freestyle)
                {
                    ...
                    rb.AddTorque(Vector3.up * steerzoAngle * 4 * airTimeSettings.airTimeRotationSensitivity, ForceMode.Impulse);
